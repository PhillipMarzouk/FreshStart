...
# --------------------------------------------------------------------------------
# Checkout View: Create UserOrder, calculate milk items, send notifications
# --------------------------------------------------------------------------------
class CheckoutView(View):
    def post(self, request):
        """
        Processes checkout: ensures school is selected, cart is not empty.
        Creates a new UserOrder, calculates required milk by meal type & quantity,
        creates corresponding OrderItem entries (including milk items), deletes cart,
        sends notification emails via SendPulse, and redirects to 'past_orders'.
        """
        ...
        notes = (
            request.POST.get("notes", "").strip() or request.session.get("cart_notes", "")
        )
        order = UserOrder.objects.create(
            user=request.user, school=school, status="Pending", notes=notes
        )

        # Map meal types to the corresponding milk distribution dictionary
        meal_type_to_field = {
            ...
            "Vegan": {"Soy Milk": 100},  # Vegan always gets soy
        }

        # Aggregate milk quantities per consumption date
        milk_by_date = defaultdict(lambda: defaultdict(float))

        for item in cart_items:
            meal_type = item.menu_item.meal_type
            dist = meal_type_to_field.get(meal_type)
            if not dist:
                # If a meal type isn't mapped, skip milk calculation
                print(f"⚠️ No milk distribution found for meal type: '{meal_type}'")
                continue

            # Use date_override if set, otherwise fallback to menu_item.available_date
            consumption_date = item.date_override or item.menu_item.available_date
            for milk_name, percent in dist.items():
                milk_by_date[consumption_date][milk_name] += (percent / 100) * item.quantity

        # Create OrderItem entries for milk
        for date, milk_totals in milk_by_date.items():
            for milk_name, qty in milk_totals.items():
                final_qty = round(qty)
                if final_qty == 0:
                    continue
                # Get or create a MenuItem for this milk type
                milk_item, _ = MenuItem.objects.get_or_create(
                    plate_name=milk_name, defaults={"meal_type": "Milk"}
                )
        ...
        # Create OrderItem entries for each original cart item
        for cart_item in cart_items:
            menu_item_date = cart_item.date_override or cart_item.menu_item.available_date
            OrderItem.objects.create(
                order=order,
        ...
        # Clear out the cart now that the order is processed
        cart_items.delete()

        # Send notification emails via SendPulseAPI
        SendPulseAPI().send_order_notification(order)
        SendPulseAPI().send_order_confirmation_to_customer(order)

        # Remove any saved cart_notes from session
        if "cart_notes" in request.session:
            del request.session["cart_notes"]

        messages.success(
            request, f"Your order for {school.name} has been placed and is now pending."
        )
        return redirect("past_orders")

        # The following block is unreachable due to the return above, but left as reference:
        hot_count = 0
        cold_count = 0
        for item in cart_items:
            meal_type = item.menu_item.meal_type
            if meal_type in ["Hot Meal", "Hot Vegetarian"]:
                hot_count += item.quantity
            elif meal_type in ["Cold Meal", "Cold Vegetarian"]:
                cold_count += item.quantity

        if hot_count > 0 and cold_count > 0:
            return JsonResponse(
                {"error": "You cannot order both a hot and cold meal for the same day."},
                status=400,
            )
